...
def read_qr(buffer_coordinates, camera_frame):
    """
    Функция считывания qr-кодов и преобразования считанной информации в координаты.
    В ходе полета дрон будет перемещаться по координатам, полученным из qr-кодов.
    :param buffer_coordinates: переменная для хранения координат, полученных с предыдущего qr-кода.
    :param camera_frame: снимок, сделанный дроном при достижении точки с qr-кодом.
    :return: в случае нахождения снимка возвращает buffer_coordinates, координаты/ключ, флаг на окончание полета, иначе None.
    """
...
def drone_flight(drone, camera_ip):
    """
    Функция полета дрона по точкам с qr-кодами и точками с буквами. Дрон стартует с первого qr-кода и, прочитав
    координаты с qr-кода, летит на две точки с буквами. Далее, сделав снимки на каждой точке с буквами,
    летит на точку со следующим qr-кодом. И так по всем точкам. На последней заданной организатором точке дрон прочтет
    qr-код, в котором будет заложен ключ от шифра, необходимый для расшифровки слова, собранного с начала полета.
    :param drone: экземпляр класса Pioneer.
    :param camera_ip: экземпляр класса Camera.
    :return: возврат данных не предусмотрен.
    """
            ...
            if finish: #если в qr-коде ключ от шифра
                ...
                if list_of_cor[2] != 0 or list_of_cor[3] != 0:
                    ...
                    while True:
                        ...
                        if drone.point_reached(): #если заданная точка достигнута
                            drone.led_control(r=0, g=255, b=0) #подсвечиваем процесс съемки
                            time.sleep(2)
